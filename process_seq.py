...
SRC_HOME = "/home/yayang/phylogenomic_dataset_construction/" # where the scripts are
APPS_HOME = "/home/yayang/apps/" # where trinity and trnasdecoder dirs are located

TruSeq_ADAPTER = SRC_HOME+ "data/TruSeq_adapters"
BLASTP_DB_PATH = SRC_HOME+"data/blastp_database/db" # the custom blast database
TRINITY_CMD = APPS_HOME+"trinityrnaseq/Trinity"
TRANSDECODER_PFEFIX = APPS_HOME+"TransDecoder-3.0.0/TransDecoder."
...
def run_trinity(input_string,taxonID,num_cores,max_memory_GB,stranded=False,clip=True):
	...
	if fasta_ok(transcripts) or os.path.exists(transcripts+".gz"):
		...
		cmd += " --max_memory "+max_memory_GB+"G --CPU "+str(num_cores)
		cmd += " --full_cleanup"
...
def run_transdecoder_blastp(taxonID,num_cores,stranded=False,pfam=False):
	...
	if fasta_ok(outpep) and fasta_ok(outcds): print "Skip transdecoder"
		...
		if blastpout_ok(blastpout): print "Skip blastp"
			...
			assert os.path.exists(BLASTP_DB_PATH),"cannot fine "+BLASTP_DB_PATH
